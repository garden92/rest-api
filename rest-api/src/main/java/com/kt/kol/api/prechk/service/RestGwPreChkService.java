	...
	public <T> Mono<ResponseStdVO<DummyDTO>> ckeckBeforeRoute(T inDTO, HttpServletRequest request) {
		...
		return bmonSender.sendBmonMot("T", inDTO, new TrtErrInfoDTO("I", "", "", ""), request)
				.then(Mono.defer(() -> {
					...
					//채널ID 및 허용경로 체크
					Mono<List<KolChInfoDTO>> chChkList = kolChInfoRepository.checkKolChInfo(request.getHeader(HeaderConstants.HEADER_CHNL_TYPE), request.getHeader(HeaderConstants.HEADER_LG_DATE_TIME))
															.collectList();
					//허용IP 체크
					Mono<List<KolChIpInfoDTO>> ipChkList = kolChIpInfoRepository.checkKolChIpInfo(request.getHeader(HeaderConstants.HEADER_CHNL_TYPE), request.getRemoteAddr())
															.collectList();
					//허용사용자 체크
					Mono<List<KolChUserInfoDTO>> userChkList = kolChUserInfoRepository.checkKolChUserInfo(request.getHeader(HeaderConstants.HEADER_CHNL_TYPE), request.getHeader(HeaderConstants.HEADER_USER_ID))
															.collectList();
					
					//병렬 체크 로직 수행
					return Mono.zip(chChkList, ipChkList, userChkList)
							.flatMap(dbRslt -> {
